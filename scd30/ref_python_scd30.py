...
class SCD30:
    ...
    def __init__(self, i2c, addr, pause=1):
        self.i2c = i2c
        self.pause = pause
        self.addr = addr
    ...
    def start_continous_measurement(self, ambient_pressure=0):
        ...
        data = bint + bytes([crc])
        self.__write_command(self.START_CONT_MEASURE, data)
    ...
    def set_measurement_interval(self, interval):
        ...
        crc = self.__crc(bint[0], bint[1])
        data = bint + bytes([crc])
        self.__write_command(self.SET_MEASURE_INTERVAL, data)
    ...
    def set_temperature_offset(self, offset):
        ...
        crc = self.__crc(bint[0], bint[1])
        data = bint + bytes([crc])
        self.__write_command(self.SET_TEMP_OFFSET, data)
